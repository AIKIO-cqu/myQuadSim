...
class Control:
    ...
    def controller(self, traj, quad, sDes, Ts):
        ...
        if (traj.ctrlType == "xyz_vel"):
            self.saturateVel()  # 对速度设定点进行饱和处理，确保不超过最大速度
            self.z_vel_control(quad, Ts)  # 垂直速度控制
            self.xy_vel_control(quad, Ts)  # 水平速度控制
            self.thrustToAttitude(quad, Ts)  # 根据推力设定点计算期望的姿态
            self.attitude_control(quad, Ts)  # 姿态控制，计算从当前姿态到期望姿态所需的角速度
            self.rate_control(quad, Ts)  # 角速度控制，根据期望的角速度和当前角速度计算控制输入
    ...
    def z_vel_control(self, quad, Ts):
        ...
        if (orient == "NED"):
            thrust_z_sp = vel_P_gain[2] * vel_z_error - vel_D_gain[2] * quad.vel_dot[2] + quad.params["m"] * (
    ...
    def xy_vel_control(self, quad, Ts):
        ...
        thrust_xy_sp = vel_P_gain[0:2] * vel_xy_error - vel_D_gain[0:2] * quad.vel_dot[0:2] + quad.params["m"] * (
